...
public class Vector2D {
	...
	//CREAR UNA POSICION ALEATORIA
	public static Vector2D getPosAleatoria(double minWidth, double maxWidth, double minHeight, double maxHeight) {
		assert (maxWidth >= minWidth);
		assert (maxHeight >= minHeight);
		double x = minWidth + Utils.RAND.nextDouble(maxWidth - minWidth);
		double y = minHeight + Utils.RAND.nextDouble(maxHeight - minHeight);
		assert (x >= minWidth && x <= maxWidth);
		assert (y >= minHeight && y <= maxHeight);
		return new Vector2D(x, y);
	}
	
	//AJUSTAR UNA POSICION
	public Vector2D ajustaPosicion(double width, double height) {
